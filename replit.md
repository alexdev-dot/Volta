...
- Vite reads `PORT` env variable (defaults to 5173); the workflow sets `PORT=5000` for Replit's webview.
- After a fresh zip import, run `cd volta && CI=true pnpm install` to restore node_modules.
