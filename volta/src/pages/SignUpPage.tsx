...
export default function SignUpPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const existingUsers = JSON.parse(localStorage.getItem("volta_users") || "[]");
    ...
    localStorage.setItem("volta_users", JSON.stringify(existingUsers));

    setTimeout(() => {
      setLoading(false);
      // Set current session
      localStorage.setItem("volta_user", JSON.stringify(userData));
      localStorage.setItem("volta_session", JSON.stringify({ email: userData.email, loggedIn: true, role: userData.role }));
      
      if (role === "professional") {
        localStorage.setItem("volta_onboarding_prefill", JSON.stringify({
          firstName,
          lastName,
          phone: fields.phone.trim(),
          location: fields.location,
