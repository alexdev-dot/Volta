      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden bg-white">
            <button 
              ...
              className={`px-3 py-2 transition-colors border-r border-gray-200 ${viewMode === 'list' ? 'bg-green-600' : 'hover:bg-gray-50'}`}
            >
              <List className={`w-4 h-4 ${viewMode === 'list' ? 'text-white' : 'text-gray-400'}`} />
            </button>
            <button 
              onClick={() => setViewMode('grid')}
              className={`px-3 py-2 transition-colors ${viewMode === 'grid' ? 'bg-green-600' : 'hover:bg-gray-50'}`}
            >
              <LayoutGrid className={`w-4 h-4 ${viewMode === 'grid' ? 'text-white' : 'text-gray-400'}`} />
            </button>
          </div>
        </div>
        ...
        {viewMode === 'grid' ? (
                ...
                <div className="p-3 sm:p-5 flex-1 flex flex-col justify-between min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-2 sm:mt-3">
                    ...
                    <button 
                      ...
                      className="bg-green-600 hover:bg-green-700 text-white font-semibold text-[10px] sm:text-xs px-2 py-1 rounded-lg transition-colors"
                    >
                      View Profile
                    </button>
                  </div>
          ...
          <div className="flex flex-col gap-3 mb-8">
            {PROS.map((pro) => (
              <div key={pro.name} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all group">
                <div className="flex items-center gap-4 p-4">
                  <div className="relative w-16 h-16 flex-shrink-0">
                    <img src={pro.img} alt={pro.name} className="w-full h-full object-cover rounded-xl" />
                    <div className={`absolute -top-1 -right-1 flex items-center gap-1 ${pro.statusColor} text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="font-bold text-gray-900 text-sm truncate">{pro.name}</p>
                    ...
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <span className="text-gray-400 text-xs">{pro.distance}</span>
                    </div>
                    <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full ${pro.availColor}`}>
                      {pro.avail}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    <button 
                      onClick={() => handleViewProfile(pro)}
                      className="bg-green-600 hover:bg-green-700 text-white font-semibold text-xs px-3 py-1.5 rounded-lg transition-colors"
                    >
                      View Profile
                    </button>
                  </div>
